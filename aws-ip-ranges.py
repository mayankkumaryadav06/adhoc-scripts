...
def prefixes(includes=None, excludes=None, regions=None):
    ...
    def valid_service(services):
        for exclude in excludes:
            if exclude in services:
                return False
    ...
    for ip_prefix, _regions, services in merge_prefixes(payload):
        if valid_region(regions):
            if valid_service(services):
                filtered.append(ip_prefix)

    return filtered

if __name__ == '__main__':
    print(prefixes(includes=['AMAZON'], excludes=['EC2'], regions=['eu-west-2']))
